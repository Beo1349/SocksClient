...
{
    class StoragePresenter : IPresenter
    {
        IBaseModel<CStorage> model;
        IItemView<CStorage> view;
        CStorage current;

        public StoragePresenter(IBaseModel<CStorage> model, IItemView<CStorage> view)
        {
            this.model = model;
            this.view = view;
            model.Reload();
